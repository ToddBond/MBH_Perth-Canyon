...
bathy <- raster(paste(r.dir, "PC_BEST_UTM_survey.tif", sep='/'), col=heat.colors)
#bathy[bathy > -1500] <- NA
plot(bathy)
bathy.df <- as.data.frame( rasterToPoints( bathy))

## Define 4 depth categories to sample within
depthBins <- c(-5100, -3500,- 2000, -1000)
bathy.df$category <- cut( bathy.df$PC_BEST_UTM_survey, depthBins, include.lowest=TRUE)
...
title( main="Perth Canyon Bathymetry (m)", xlab="x", ylab="y")
...
proj4string(unevenSample)<- CRS( "+proj=utm +zone=50 +south +datum=WGS84 +units=m +no_defs")
#LLcoor<-spTransform(WGScoor,CRS("+proj=longlat"))
raster::shapefile(unevenSample, "PC_stratified-single.shp", overwrite = TRUE)

#save contours as shapefile
bathy.contour <- rasterToContour(bathy, levels =depthBins)
proj4string(bathy.contour)<- CRS( "+proj=utm +zone=50 +south +datum=WGS84 +units=m +no_defs")
raster::shapefile(bathy.contour, "PC_bathy_contour.shp", overwrite = TRUE)
...
proj4string(WGScoor)<- CRS( "+proj=utm +zone=50 +south +datum=WGS84 +units=m +no_defs")
#LLcoor<-spTransform(WGScoor,CRS("+proj=longlat"))
raster::shapefile(WGScoor, "PC_TranSurveyPoints.shp", overwrite = TRUE)
